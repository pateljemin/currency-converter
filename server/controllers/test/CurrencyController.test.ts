...
describe('Currency Controller Test Suit', () => {
    ...
    it('Get Currency Rate', async () => {
        const { body } = await request(app).get("/api/v1/currency-rate/EUR-HKD");
        expect(body.rate).toEqual(9.336261);
        expect(body.symbols).toEqual(`EUR-HKD`);
    });

    it('Get Currency Rate from cache', async () => {
        const { body } = await request(app).get("/api/v1/currency-rate/USD-HKD");
        console.log(body);
        expect(body.rate).toEqual("7.767");
        expect(body.symbols).toEqual(`USD-HKD`);
    });

    it('Wrong Input', async () => {
        const { body } = await request(app).get("/api/v1/currency-rate/EUR");
        expect(body.message).toEqual('Provide valid symbols');
    });

    it('API Error', async () => {
        const mock = new MockAdapter(axios);
        mock.onGet(`${FIXER_API}?access_key=${process.env.FIXER_KEY}`).networkError();
        const { body } = await request(app).get("/api/v1/currency-rate/EUR-INR");
